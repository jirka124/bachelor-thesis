...
router.post("/view-stock-products-csr", async function (req, res) {
  let reqState = null, products = [], totalCount = 0;

  const availSorts = ["feat", "sell", "aplha-a", "aplha-z", "price-low", "price-high"]
  const inputs = {
  ...
  try {
    ...
    if(typeof inputs.sortVal !== "string" || !availSorts.includes(inputs.sortVal)) throw new Error("ZU_7BBOM4V4")
    if(typeof inputs.pageVal !== "number" || inputs.pageVal < 1) throw new Error("TIE1ER13_VE")
  ...
  if(inputs.sortVal === "sell") {
    ...
    if(orderArr.length > 0) orderBy = `CASE WHEN product_id IN (${orderArr.join(",")}) THEN 1 ELSE 2 END, FIELD(product_id, ${orderArr.join(",")})`
...
router.post("/view-product-ssr", async function (req, res) {
  let reqState = null, product = null;
  const inputs = {
    productId: +req.body.productId || null,
  }

  try {
    if(typeof inputs.productId !== "number" || inputs.productId < 1) throw new Error("RIG45BLE_L")
    let r = await utilPre.retQueryOne(
      `SELECT product_id as productId, name, description, price FROM product WHERE product_id = ?`,
      [inputs.productId],
      shared.Connect.connWrap
    );
    if (r.state) {
    ...
    r = await utilPre.retQuery(
      `SELECT f_key AS fKey, f_value as fValue FROM product_feature WHERE product_id = ?`,
      [inputs.productId],
      shared.Connect.connWrap
    );
    if (r.state) {
...
router.post("/view-product-csr", async function (req, res) {
  let reqState = null, similar = [], reviews = [];
  const inputs = {
    productId: +req.body.productId || null,
  }

  try {
    if(typeof inputs.productId !== "number" || inputs.productId < 1) throw new Error("HU4E_NE36AEB")
    let r = await utilPre.retQuery(
...
router.post("/product-add-review", async function (req, res) {
  let reqState = null, addedReview = null;
  const inputs = {
    productId: +req.body.productId || null,
  ...
  try {
    if(typeof inputs.productId !== "number" || inputs.productId < 1) throw new Error("TUDLEN78_EIH")
    if(typeof inputs.stars !== "number" || inputs.stars < 1 || inputs.stars > 5) throw new Error("KE35E_BIE48A")
    if(typeof inputs.text !== "string" || inputs.text.length < 1 || inputs.text.length > 500) throw new Error("JFU8EAMB6E")
    ...
    if (r.state) {
      addedReview = { productReviewId: r.result.insertIds[0], productId: inputs.productId, stars: inputs.stars, text: inputs.text }
