...
import { api, createAPI } from "@/plugins/02.axios.js";
...
export default defineComponent({
  ...
  async setup() {
    ...
    const userId = useRoute().params.userId || null;
    ...
    const fetchPageSSR = async (userId) => {
      ...
      try {
        ...
        if (process.server && !nuxtApp.isHydrating)
          clearNuxtData("user/view-user-profile-ssr");

        const { data, error } = await useAsyncData(
          "user/view-user-profile-ssr",
          () => api.post("user/view-user-profile-ssr", reqBody),
          { pick: ["data"] }
        );
        r = data._value.data;
  ...
  computed: {
    ...
    userSince() {
      return this.userProfile.signupDate ? `Member of SoConnect since ${this.userProfile.signupDate.getFullYear()}` : "Information is LOCKED 🔒";
    },
    userPosts() {
