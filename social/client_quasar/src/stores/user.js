  ...
  actions: {
    ...
    setFeedPosts(posts, likes, likeCounts, replyCounts) {
      ...
      if (likes && likes.length > 0)
        posts.map(
          (p) =>
            (p.hasLiked = Boolean(likes.find((l) => l.postId === p.postId)))
        );
    ...
    setLikeOnPost(likeObj, route) {
      ...
      let post;
      if (route.name === "feed")
        post = this.feedPosts.find((fp) => fp.postId === likeObj.postId);
      else if (route.name === "view-user")
        post = this.viewedUserProfilePosts.find(
          (fp) => fp.postId === likeObj.postId
        );
    ...
    setPostComments(commentObj, route) {
      ...
      let post;
      if (route.name === "feed")
        post = this.feedPosts.find((fp) => fp.postId === commentObj.postId);
      else if (route.name === "view-user")
        post = this.viewedUserProfilePosts.find(
          (fp) => fp.postId === commentObj.postId
        );
