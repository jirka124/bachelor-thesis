...
export default route(function ({ store, ssrContext }) {
  ...
  Router.beforeEach(async (to) => {
    ...
    const loggedIntercept = ({ data }) => {
      const userStore = useUserStore(store);
      if (data && data.hasOwnProperty("isLogged"))
        userStore.setIsLogged(data.isLogged);
    };

    // define own interceptors run before nav guard resolution
    hybridRoutes.addIterceptURL("/app**", loggedIntercept);
    hybridRoutes.addIterceptURL("/app/user/*", loggedIntercept);
